...
Date: August 2021
Description:
    Remote client for multi-camera recording prepared for ZED 2 camera and Ardusimple
    GNSS receiver.

Use:

    remote_zed_gnss_config_obj = ClientConfig(path_client_config_file)
    remote_client_zed_gnss_obj = RemoteClientZedGNSS(remote_zed_gnss_config_obj, my_zed_device_configuration, my_gnss_device_configuration)
...
from gnss_classes.job_thread_gnss import JobThreadGNSS


class RemoteClientZedGNSS:
    _remote_config = None
    _token = None
    ...
    _gnss_device_configuration = None
    last_command = ''

    def __init__(self, client_config_param, camera_device_configuration, gnss_device_configuration=None):
        ...
        if gnss_device_configuration is None:
            logging.debug('gnss_device_configuration is empty')
        else:
            self.gnss_device_configuration = gnss_device_configuration
    ...
    def run(self):
        ...
        job_thread_gnss = JobThreadGNSS(self.gnss_device_configuration)
        # ---------------------------------------------
        ...
        try:
            ...
            while True:
                ...
                if command == CmdConfig.enable_remote_client:
                    ...
                    if job_thread_gnss.flag_enabled is True:
                        job_thread_gnss = JobThreadGNSS(self.gnss_device_configuration)
                        job_thread_gnss.start()
                    else:
                        print('There is a job GNSS recording now')
                    ...
                    if job_thread_gnss.flag_enabled is True:
                        # if job_thread_gnss.is_alive():
                        job_thread_gnss.shutdown_flag.set()
                        job_thread_gnss.join()
                    else:
                        print('There is not an active job gnss')
