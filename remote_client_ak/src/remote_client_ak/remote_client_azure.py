...
Date: August 2021
Description:
  Project: Remote client for multi-camera recording prepared for Azure Kinect camera.
...
import uuid
import configparser
from src.remote_client_ak.cmd_config import CmdConfig
from src.remote_client_ak.host_info_register import HostInfoRegister
from src.camera_classes.job_thread_azure import JobThreadAzure


class RemoteClientAzure:
    _remote_config = None
    _token = None
    ...
    _device_configuration = None
    last_command = ''
    def __init__(self, config_param, device_configuration):
    ...
    def run(self):
        ...
        # ---------------------------------------------
        jobCamera1 = JobThreadAzure(self._device_configuration, self._remote_config.path_video_output)
        # ---------------------------------------------
        ...
        try:
            ...
            while True:
                ...
                if command == CmdConfig.enable_remote_client:
                    ...
                    self.last_command = command
                    recording_flag = True
                    if not jobCamera1.is_alive():
                        jobCamera1 = JobThreadAzure(self._device_configuration, self._remote_config.path_video_output)
                        jobCamera1.start()
                    else:
                        print('There is a job recording now')
                    ...
                    recording_flag = False
                    waiting_flag = True
                    if jobCamera1.is_alive():
                        jobCamera1.shutdown_flag.set()
                        jobCamera1.join()
                    else:
                        print('There is not an active job')
                    ...
                    print('*** You must enable clients from the central console !!! ***')
                    self.last_command = command
                    break
                else:
                    print(f'CHECKING ORDERS... LAST ORDER EXECUTED -> {self.last_command}')
                # ----------------------------------
